...
- 结构化 PII（SSN / 电话 / Email / 账号 / 保单号 / Member ID）
- 裸姓名、OCR 粘连姓名
- 地址 / 邮编
- 图片级遮盖验证（不只测「文字里还剩什么」，要测「图片上真的被涂黑了吗」）

**P1 · OCR 粘连导致的 PII 漏检** —— 不先堆 NER

`JENNIFERWASHINGTON`/`JOHNBDOE`/`iJANEDOE`/`JAMES&KARENQ.HINDS` 这四个
反复出现的失败样本，诊断过（不是本项目行重建逻辑的锅，是 PaddleOCR 检测/
识别阶段自己把这几个词粘成一个框）。NER A/B 测过一次轻量方案，0/4 抓不到
这类粘连（决定 12 之前那次任务）。下一步该研究的是：「bbox 很宽 + 全大写 +
疑似姓名」这类可疑结果，能不能在本地做局部二次 OCR / 重新切分——这也是
旧清单里「二次 OCR 存疑」条目的下落，之前测的是「二次 OCR 提升准确率」，
没测出效果（`improved: 0`），这次要测的是「二次 OCR 专门解决粘连」，是
不同的问题，不能用旧结论直接判死刑。
