...
let score={cat:[0,0],amt:[0,0],act:[0,0],date:[0,0],sent:[0,0]};
const rows=[];
...
for (const [name, truth] of Object.entries(gt)) {
  ...
  // 发信日期是独立的一维 —— 和到期日混为一谈是最容易害老人做错事的一类错
  const sentGot = f.statementDate && f.statementDate.trusted ? f.statementDate.value : null;
  const sentOk = sentGot === (truth.statement_date ?? null);
  ...
  score.sent[0]+=sentOk?1:0; score.sent[1]++;
  rows.push({name,catOk,catGot,amtOk,amtGot,amtWant:truth.amount_shown,actOk,actGot,actWant:truth.payment_action,dateOk,dateGot,dateWant:truth.due_date,sentOk,sentGot,sentWant:truth.statement_date??null});
}

const ACT={pay:'要交',autopay:'自动扣款',none:'不用交'};
console.log('图'.padEnd(24)+'类别'.padEnd(20)+'金额'.padEnd(18)+'付款方式'.padEnd(20)+'到期日'.padEnd(24)+'发信日期');
...
for(const r of rows){
  console.log(
    ...
    ((r.dateOk?'✅ ':'❌ ')+(r.dateGot||'无')+(r.dateOk?'':` (应为${r.dateWant||'无'})`)).padEnd(24)+
    ((r.sentOk?'✅ ':'❌ ')+(r.sentGot||'无')+(r.sentOk?'':` (应为${r.sentWant||'无'})`)));
}
console.log('-'.repeat(130));
const pct=a=>`${a[0]}/${a[1]} ${Math.round(a[0]/a[1]*100)}%`;
console.log(`类别 ${pct(score.cat)}   金额 ${pct(score.amt)}   付款方式 ${pct(score.act)}   到期日 ${pct(score.date)}   发信日期 ${pct(score.sent)}`);
