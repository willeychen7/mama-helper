...
export const findDatesInText = (text, options = {}) => {
   ...
   *
   * 月和日、日和年之间原来都要求 \s+（至少一个空格）。
   * IRS CP501/CP503 官方样本「January 28,2019」逗号后面不带空格；
   * CP504 更狠，「January30,2019」连月份和日期之间的空格都被吃掉了——
   * PP-OCR 经常把空格吃掉，跟这个项目里其他好几处 \b 断裂是同一类问题。
   * 两处都改成 \s*（允许零个空格）。
   *
   * 不怕因此误判：这里只是把「像日期的形状」抓出来，抓完之后
   * 下面立刻用 MONTH_NAMES 查表验证——只有 match[1] 真的是月份名
   * 缩写才会被采信，不是月份名的词（哪怕形状对上了）会因为
   * `month` 是 undefined 直接被跳过，所以放宽空格要求不会把
   * 「Room30,2019」这种不相关的文字误认成日期。
   */
  const wordRe =
